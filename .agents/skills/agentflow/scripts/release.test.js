...
const copy_source = source_root => {
  ...
  fs.copyFileSync(path.join(repo_root, 'CHANGELOG.md'), path.join(source_root, 'CHANGELOG.md'))
...
test('explicit assembly keeps the approved license and creates the exact public payload', () => {
  ...
  try {
    ...
    assert.deepEqual(fs.readdirSync(destination).sort(), ['.claude-plugin', 'CHANGELOG.md', 'LICENSE', 'README.md', 'README.zh-tw.md', 'skills'])
    assert.equal(fs.readFileSync(path.join(destination, 'CHANGELOG.md'), 'utf8'), fs.readFileSync(path.join(source_root, 'CHANGELOG.md'), 'utf8'))
    ...
    for (const readme of [english, chinese]) {
      assert.doesNotMatch(readme, /\{\{(?:org|repo|version)\}\}/)
      assert.match(readme, /Node(?:\.js)? 18|Node 18/i)
      assert.match(readme, /Git/i)
      assert.match(readme, /agf setup/)
      assert.match(readme, /godev/)
    }
    assert.match(english, /node "\$HOME\/\.codex\/skills\/agentflow\/scripts\/setup\.js"/)
    assert.match(english, /node "\$HOME\/\.claude\/skills\/agentflow\/scripts\/setup\.js"/)
    ...
    const skill = fs.readFileSync(path.join(destination, 'skills', 'agentflow', 'SKILL.md'), 'utf8')
    const version = skill.match(/^  version: "(\d+\.\d+\.\d+)"$/m)[1]
    assert.ok(skill.includes(`# Agentflow v${version}`))
    assert.equal(plugin.version, version)
    assert.ok(english.startsWith(`# Agentflow v${version}\n`))
    assert.ok(chinese.startsWith(`# Agentflow v${version}\n`))
    assert.deepEqual(Object.keys(plugin).sort(), ['author', 'description', 'name', 'version'])
