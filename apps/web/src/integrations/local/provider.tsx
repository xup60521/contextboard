...
type LocalDatabaseState =
  | { status: "opening"; database: ContextboardDatabase; workspaceId: null; deviceId: null; error: null }
...
export function LocalDatabaseProvider({ children }: { children: ReactNode }) {
  ...
  const [state, setState] = useState<LocalDatabaseState>({ status: "opening", database, workspaceId: null, deviceId: null, error: null });
  ...
  return <LocalDatabaseContext.Provider value={state}>{children}</LocalDatabaseContext.Provider>;
}

export function useLocalDatabase() {
  const state = useContext(LocalDatabaseContext);
  if (!state) throw new Error("useLocalDatabase must be used inside LocalDatabaseProvider");
  return state;
}
