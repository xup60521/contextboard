...
let currentSearch = {
	orphan: "",
	sort: "updated" as "updated" | "updated_asc" | "title" | "title_desc",
	q: "",
};
...
const fromRouteSort: Record<typeof currentSearch.sort, CardSortOrder> = {
	updated: "updated_desc",
	updated_asc: "updated_asc",
	title: "title",
	title_desc: "title_desc",
};
const toRouteSort: Record<CardSortOrder, typeof currentSearch.sort> = {
...
function TestPage() {
	...
	const runtime: ApplicationRuntime = {
		...
		cards: {
			list(options = {}) {
				const sortBy = toRouteSort[options.sortBy ?? "updated_desc"];
				const query = usePaginatedQueryMock(
					"cards.listAll",
					{
						...(options.searchTerm ? { searchTerm: options.searchTerm } : {}),
						...(options.orphanOnly ? { orphanOnly: true } : {}),
						sortBy,
					},
					{ initialNumItems: 50 },
				);
				return query.results.map((card: Record<string, unknown>) => ({
					id: String(card._id),
					title: String(card.derivedTitle),
					preview: String(card.preview ?? ""),
					createdAt: Number(card._creationTime ?? 1),
					updatedAt: Number(card.updatedAt ?? 1),
					version: 1,
					activePlacementCount: Number(card.activePlacementCount ?? 0),
				})) as never;
			},
			get: async () => null,
			create: async () => "new-card",
			updateContent: async () => 1,
		...
		whiteboards: {
			...
			get: async () => null,
			createRoot: async () => "",
			createSubwhiteboard: async () => ({ itemId: "", childWhiteboardId: "" }),
			rename: async () => "",
